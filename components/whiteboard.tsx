...
export const Whiteboard = () => {
  ...
  // Combine gestures - drawing takes priority over pan/pinch
  const composed = Gesture.Simultaneous(drawGesture, Gesture.Simultaneous(pinch, pan))
  ...
  const renderPath = (stroke: any, key: number) => {
    ...
    return <Path key={key} d={d} stroke={stroke.tool === 'eraser' ? '#fff' : '#000'} strokeWidth={stroke.strokeWidth} fill="none" strokeLinecap="round" strokeLinejoin="round" />
  }

  return (
    <View style={styles.container}>
      <ViewShot ref={viewShotRef} options={{ format: 'png', quality: 1.0, result: 'tmpfile' }} style={styles.canvas}>
          ...
          <Animated.View style={[styles.canvas, animatedStyle]}>  
            <Svg style={styles.svg}>
              ...
              {tool === 'eraser' && indicatorPosition && (
                <Circle cx={indicatorPosition.x} cy={indicatorPosition.y} r={eraserSize / 2} fill="rgba(200,200,200,0.5)" stroke="gray" strokeWidth="1" />
              )}
            </Svg>
          </Animated.View>
        </GestureDetector>
      ...
      <View style={styles.actionContainer}>
        <TouchableOpacity style={styles.actionButton} onPress={undo}><Text style={styles.actionText}>Undo</Text></TouchableOpacity>
        <TouchableOpacity style={styles.actionButton} onPress={redo}><Text style={styles.actionText}>Redo</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.actionButton, styles.clearButton]} onPress={clearAll}><Text style={styles.actionText}>Clear All</Text></TouchableOpacity>
      </View>

      <TouchableOpacity style={[styles.actionButton, { backgroundColor: '#007AFF' }]} onPress={async () => {
...
const styles = StyleSheet.create({
  ...
  canvas: { flex: 1 },
  svg: { flex: 1 },
  toolContainer: { position: 'absolute', top: 50, left: '50%', transform: [{ translateX: -85 }], flexDirection: 'row', gap: 10, backgroundColor: 'rgba(255,255,255,0.8)', padding: 8, borderRadius: 12 },
  ...
  sizeContainer: { position: 'absolute', top: 110, left: 20, right: 20, flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.8)', padding: 8, borderRadius: 8, gap: 10 },
  sizeLabel: { fontSize: 14, fontWeight: '600', marginRight: 5 },
  sizeButton: { width: 30, height: 30, justifyContent: 'center', alignItems: 'center', backgroundColor: '#e0e0e0', borderRadius: 15, borderWidth: 2, borderColor: 'transparent' },
  activeSizeButton: { borderColor: '#007AFF' },
  sizeButtonText: { fontSize: 12, fontWeight: 'bold', color: '#333' },
  actionContainer: { position: 'absolute', bottom: 40, left: '50%', transform: [{ translateX: -125 }], flexDirection: 'row', gap: 15 },
  actionButton: { paddingHorizontal: 16, paddingVertical: 10, backgroundColor: '#f0f0f0', borderRadius: 8, borderWidth: 1, borderColor: '#ccc' },
